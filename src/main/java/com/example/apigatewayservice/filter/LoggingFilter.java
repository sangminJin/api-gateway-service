...
//
//            log.info("Logging Filter baseMessage: baseMessage -> {}", config.getBaseMessage());
//
//            if(config.isPreLogger()) log.info("Logging Filter Start: request id -> {}", request.getId());
//
//            // Custom Post Filter
//            return chain.filter(exchange).then(Mono.fromRunnable(() -> {
//                if(config.isPostLogger()) log.info("Logging Filter End: response code -> {}", response.getStatusCode());
//            }));
//        };
